...
- **跨局知识库**：每局结束模板化提炼经验条目，下局开局注入玩家 persona；支持分区存储（wolf/good-faction/seer/witch 等）与淘汰策略。
- **人类席位 + 浏览器面板**：`humanSeat` 随机分配人类座位，通过 HTTP 面板行动（发言/投票/夜间技能），与 agent 玩家完全同场竞技。
...
| 工具 | 用途 | 调用者 |
|---|---|---|
| `werewolf_start` | 开局：随机分配身份、生成 9 个玩家 agent、进入第一夜。不传 `humanSeat` 则自动随机 1-9 分配人类座位。 | 主持人 |
...
详见 [`固化复盘与避坑指南.md`](./固化复盘与避坑指南.md)。关键项：

1. **dsh-tools 多副本** → ESM Symbol 身份不匹配 → `Cannot read properties of undefined (reading 'prepare')`。解法：依赖用 `link:` 指向全局唯一副本。
2. **未声明 inject** → `apply` 早于服务就绪 → 工具永不注册。解法：声明 `inject: ['subagents', 'agents', 'tools', 'webServer']`。
3. **固化版用 harness.\*** → 沙箱 builtin 在正式插件不存在。解法：改用 `defineTool` + `ctx.tools.register`。
4. **client RPC 不存在** → 正式插件无 `harness.handle`。解法：人类面板走 webServer HTTP 路由。
