...
return {
  apply(ctx) {
    ...
    registerTool({
      ...
      async execute(_args, exec) {
        ...
        try {
          ...
          pushEvent('游戏开始。本局配置：3狼人、1预言家、1女巫、1猎人、3平民。' + (humanSeat !== null ? '人类玩家坐' + humanSeat + '号位（' + ROLE_NAMES[humanRole] + '），请通过界面行动。' : '') + (released ? '（已释放上一局 ' + released + ' 个玩家agent）' : ''))
    ...
    // ── Client RPC ──
    const rpcDisposers = []
    rpcDisposers.push(harness.handle('werewolf.uiState', async () => {
      if (!game) return { ok: false, error: '没有进行中的游戏' }
      const g = game
      let progress = null
    ...
    rpcDisposers.push(harness.handle('werewolf.humanAct', async (args) => {
      if (!game) return { ok: false, error: '没有进行中的游戏' }
    ...
    rpcDisposers.push(harness.handle('werewolf.abort', async () => {
      if (!game) return { ok: false, error: '没有进行中的游戏' }
      const g = game
      game = null
      const inbox = g.humanInbox || []
