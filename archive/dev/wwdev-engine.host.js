...
return {
  apply(ctx) {
    ...
    const KNOWLEDGE_PATH = 'D:/agentWorkSpace/werewolf-engine/knowledge.json'
    ...
    async function loadKnowledge() {
      ...
      try {
        const fs = ctx.get('fs')
        const t = await fs.resolve(KNOWLEDGE_PATH, { cwd: 'D:/agentWorkSpace' })
    ...
    async function finishReview() {
      ...
      try {
        await writeFile('D:/agentWorkSpace/werewolf-engine/last-review.json', JSON.stringify({
          finishedAt: Date.now(),
          win: g.win,
          day: g.day,
    ...
    registerTool({
      name: 'werewolf_start',
      description: '（狼人杀）开始一局9人狼人杀：随机分配3狼1预1女1猎3民，生成9个玩家agent并进入第一夜。开局前自动释放上一局遗留的玩家agent。可选 humanSeat 参数指定人类玩家座位（该座位由你通过界面扮演，不生成agent）。仅由主持人调用。',
      parameters: {
        humanSeat: { type: 'number', description: '人类玩家座位号 1-9；缺省或0=纯agent局' },
      },
      output: OUTPUT,
      timeoutMs: 600000,
      async execute(_args, exec) {
        ...
        const humanSeat = (typeof _args.humanSeat === 'number' && Number.isInteger(_args.humanSeat) && _args.humanSeat >= 1 && _args.humanSeat <= 9) ? _args.humanSeat : null
        ...
        try {
          ...
          if (humanSeat !== null) {
            ...
            if (hp) {
              try { await deliverTo(hp, '你坐在' + humanSeat + '号位，本局身份：' + ROLE_NAMES[humanRole] + '。请通过界面行动，引擎会在轮到你时通知你。') } catch (e) {}
            }
          }
          ...
          return { ok: true, phase: game.phase, releasedOld: released, humanSeat, roles: game.players.map((p) => ({ seat: p.seat, role: p.role, childId: p.agentId, human: !!p.human })) }
        } catch (e) {
    ...
    // ── Client RPC（harness.handle 兼容 client 半）+ 独立面板 API（webServer 路由）──
    const rpcDisposers = []
    ...
    rpcDisposers.push(harness.handle('werewolf.uiState', () => buildUiState()))
    rpcDisposers.push(harness.handle('werewolf.humanAct', (args) => handleHumanAct(args)))
    rpcDisposers.push(harness.handle('werewolf.abort', () => handleAbort()))
      ...
      try {
        rpcDisposers.push(_ws.register({
          ...
          handler: async (req, res) => {
            try {
              const fs = ctx.get('fs')
              const t = await fs.resolve('D:/agentWorkSpace/werewolf-ui/panel.html', { cwd: 'D:/agentWorkSpace' })
    ...
    console.log('[werewolf] plugin v32 active (msg classify + concurrent deliver)')
    return () => {
